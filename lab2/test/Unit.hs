...
solveSlaeTest =
  test
    ...
    , "testEmpty" ~: "(solveSlae [])" ~: [] ~=? solveSlae []
    ]

solveSlaeRecursiveTest :: Test
solveSlaeRecursiveTest =
  test
    [ "testSolRec" ~: "(solveSlaeRecursive" ++
      ...
      solveSlaeRecursive
        ...
        []
    , "testEmpty" ~: "(solveSlaeRecursive [] [])" ~: [] ~=? solveSlaeRecursive [] []
    ]

gaussianEliminationTest :: Test
gaussianEliminationTest =
  test
    [ "testGaussianElimination" ~: "(gaussianElimination" ++
      "[ [41, 87, 4, 0, 34, 16, 8, 0, 45, 55]" ++
      ", [77, 12, 55, 96, 21, 46, 18, 99, 18, 0]" ++
      ", [8, 27, 66, 43, 92, 8, 79, 67, 0, 15]" ++
      ", [35, 47, 54, 47, 77, 73, 3, 47, 92, 41]" ++
    ...
    , "testEmpty" ~: "(gaussianElimination [])" ~: [] ~=? gaussianElimination []
    ]

stepToUpperTriangularFormTest :: Test
stepToUpperTriangularFormTest =
  test
    [ "testStepToUpperTriangularForm" ~: "(stepToUpperTriangularForm" ++
      "[ [42, 77, 9, 20, 9, 0, 40, 89, 53, 62]" ++
      ", [40, 74, 74, 96, 11, 39, 78, 47, 29, 0]" ++
      ", [28, 0, 31, 53, 0, 57, 2, 98, 20, 89]" ++
      ", [51, 44, 0, 71, 33, 83, 28, 33, 20, 77]" ++
      ...
      stepToUpperTriangularForm
        ...
        0
    , "testEmpty" ~: "(stepToUpperTriangularForm [] 0)" ~: [] ~=? stepToUpperTriangularForm [] 0
    ]

subtractAlignedPivotRowTest :: Test
subtractAlignedPivotRowTest =
  test
    [ "testRow" ~: "(subtractAlignedPivotRow [70, 10, 16, 76, 65, 88, 98, 3] 3 [12, 59, 98, 31, 35, 26, 63, 41])" ~:
      [ -16.552631578947366
      , 54.921052631578945
      , 91.47368421052632
      , 0.0
      ...
      subtractAlignedPivotRow [70, 10, 16, 76, 65, 88, 98, 3] 3 [12, 59, 98, 31, 35, 26, 63, 41]
    , "testEmpty" ~: "(subtractAlignedPivotRow [] 0 [])" ~: [] ~=? subtractAlignedPivotRow [] 0 []
    ]
...
main = do
  ...
  runTestTT solveSlaeTest
  runTestTT solveSlaeRecursiveTest
  runTestTT gaussianEliminationTest
  runTestTT stepToUpperTriangularFormTest
  runTestTT subtractAlignedPivotRowTest
